object frmCadMovtos: TfrmCadMovtos
  Left = 467
  Top = 264
  Width = 655
  Height = 500
  Caption = 'Movimenta'#231#245'es Financeiras - v.02'
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  OnActivate = FormActivate
  OnClose = FormClose
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object Label7: TLabel
    Left = 8
    Top = 264
    Width = 36
    Height = 13
    Caption = 'Carteira'
  end
  object edtTipoInvest: TLabel
    Left = 248
    Top = 264
    Width = 24
    Height = 13
    Caption = 'Ativo'
  end
  object Label3: TLabel
    Left = 559
    Top = 263
    Width = 71
    Height = 13
    Caption = 'Data do Movto'
  end
  object Label6: TLabel
    Left = 248
    Top = 312
    Width = 27
    Height = 13
    Caption = 'Cotas'
  end
  object Label8: TLabel
    Left = 553
    Top = 312
    Width = 73
    Height = 13
    Caption = 'Total do Aporte'
  end
  object Label1: TLabel
    Left = 344
    Top = 312
    Width = 63
    Height = 13
    Caption = 'Valor Unit'#225'rio'
  end
  object Label5: TLabel
    Left = 442
    Top = 312
    Width = 81
    Height = 13
    Caption = 'Outras Despesas'
  end
  object GroupBox1: TGroupBox
    Left = 8
    Top = 8
    Width = 625
    Height = 105
    Caption = 'MOVIMENTA'#199#213'ES'
    TabOrder = 0
    object Label2: TLabel
      Left = 8
      Top = 56
      Width = 24
      Height = 13
      Caption = 'Ativo'
    end
    object Label4: TLabel
      Left = 294
      Top = 56
      Width = 36
      Height = 13
      Caption = 'Carteira'
    end
    object btnSair: TBitBtn
      Left = 540
      Top = 21
      Width = 75
      Height = 33
      Caption = 'Sai'
      TabOrder = 2
      OnClick = btnSairClick
      Glyph.Data = {
        
        F8F8FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      NumGlyphs = 2
    end
    object dbnMovto: TDBNavigator
      Left = 8
      Top = 24
      Width = 224
      Height = 25
      DataSource = DMPrincipal.DSMovimento
      VisibleButtons = [nbFirst, nbPrior, nbNext, nbLast]
      TabOrder = 0
      OnClick = dbnMovtoClick
      OnEnter = dbnMovtoEnter
    end
    object btnLimpar: TBitBtn
      Left = 541
      Top = 61
      Width = 74
      Height = 33
      Caption = 'Limpar'
      TabOrder = 3
      OnClick = btnLimparClick
      Glyph.Data = {
        
        FFFFFFFFFFFEFFFEFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    end
    object dbPesqAtivo: TDBLookupComboBox
      Left = 8
      Top = 73
      Width = 281
      Height = 21
      KeyField = 'cod_ativo'
      ListField = 'nom_ativo'
      ListSource = DMPrincipal.DSAtivo
      TabOrder = 1
      OnClick = dbPesqAtivoClick
    end
  end
  object btnNovo: TBitBtn
    Left = 239
    Top = 422
    Width = 75
    Height = 31
    Caption = 'Novo'
    TabOrder = 11
    OnClick = btnNovoClick
    Glyph.Data = {
      
      9E9EFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
  end
  object btnSalvar: TBitBtn
    Left = 318
    Top = 422
    Width = 75
    Height = 31
    Caption = 'Salva'
    Enabled = False
    TabOrder = 12
    OnClick = btnSalvarClick
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
  end
  object btnAlterar: TBitBtn
    Left = 398
    Top = 422
    Width = 75
    Height = 31
    Caption = 'Altera'
    TabOrder = 13
    OnClick = btnAlterarClick
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
  end
  object btnCancelar: TBitBtn
    Left = 478
    Top = 422
    Width = 75
    Height = 31
    Caption = 'Cancela'
    Enabled = False
    TabOrder = 14
    OnClick = btnCancelarClick
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
  end
  object btnExcluir: TBitBtn
    Left = 557
    Top = 422
    Width = 75
    Height = 31
    Caption = 'Exclui'
    TabOrder = 15
    OnClick = btnExcluirClick
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
  end
  object dblcbCarteira: TDBLookupComboBox
    Left = 8
    Top = 280
    Width = 225
    Height = 21
    DataField = 'cod_carteira'
    DataSource = DMPrincipal.DSMovimento
    Enabled = False
    KeyField = 'cod_carteira'
    ListField = 'nom_carteira'
    ListSource = DMPrincipal.DSCarteira
    TabOrder = 3
  end
  object dblcbAtivo: TDBLookupComboBox
    Left = 248
    Top = 280
    Width = 297
    Height = 21
    DataField = 'cod_ativo'
    DataSource = DMPrincipal.DSMovimento
    Enabled = False
    KeyField = 'cod_ativo'
    ListField = 'nom_ativo'
    ListSource = DMPrincipal.DSAtivo
    TabOrder = 4
  end
  object rbTipMovimento: TRadioGroup
    Left = 8
    Top = 304
    Width = 224
    Height = 49
    Caption = 'Tipo de Movimento '
    Columns = 2
    Enabled = False
    ItemIndex = 0
    Items.Strings = (
      'Compra'
      'Venda')
    TabOrder = 6
  end
  object dbgMovimentacoes: TDBGrid
    Left = 8
    Top = 120
    Width = 625
    Height = 137
    DataSource = DMPrincipal.DSMovimento
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
    TabOrder = 2
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'MS Sans Serif'
    TitleFont.Style = []
    OnCellClick = dbgMovimentacoesCellClick
    Columns = <
      item
        Expanded = False
        FieldName = 'Ativo'
        Width = 117
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'Carteira'
        Width = 85
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'tip_movimento'
        Title.Caption = 'C/V'
        Width = 27
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'dat_movimento'
        Title.Caption = 'Data'
        Width = 63
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'qtd_cotas'
        Title.Caption = 'Cotas'
        Width = 38
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'vlr_unitario'
        Title.Caption = 'Valor Unit'#225'rio'
        Width = 75
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'vlr_outras_despesas'
        Title.Caption = 'Outras Despesas'
        Width = 87
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'vlr_movimento'
        Title.Caption = 'Valor Total'
        Width = 90
        Visible = True
      end>
  end
  object dbeInicio: TMaskEdit
    Left = 560
    Top = 280
    Width = 72
    Height = 21
    Enabled = False
    EditMask = '!99/99/0000;1;_'
    MaxLength = 10
    TabOrder = 5
    Text = '  /  /    '
  end
  object dbeCotas: TDBEdit
    Left = 248
    Top = 328
    Width = 73
    Height = 21
    CharCase = ecUpperCase
    DataField = 'qtd_cotas'
    DataSource = DMPrincipal.DSMovimento
    Enabled = False
    TabOrder = 7
    OnExit = dbeCotasExit
  end
  object dbeValorUnitario: TDBEdit
    Left = 345
    Top = 328
    Width = 72
    Height = 21
    CharCase = ecUpperCase
    DataField = 'vlr_unitario'
    DataSource = DMPrincipal.DSMovimento
    Enabled = False
    TabOrder = 8
    OnExit = dbeValorUnitarioExit
  end
  object dbeValorAlocado: TDBEdit
    Left = 552
    Top = 328
    Width = 80
    Height = 21
    CharCase = ecUpperCase
    DataField = 'vlr_movimento'
    DataSource = DMPrincipal.DSMovimento
    Enabled = False
    TabOrder = 10
    OnEnter = dbeValorAlocadoEnter
  end
  object dbPesqCarteira: TDBLookupComboBox
    Left = 303
    Top = 80
    Width = 226
    Height = 21
    DataField = 'cod_carteira'
    Enabled = False
    KeyField = 'cod_carteira'
    ListField = 'nom_carteira'
    ListSource = DMPrincipal.DSPesqCarteira
    TabOrder = 1
    OnClick = dbPesqCarteiraClick
  end
  object dbeOutrasDespesas: TDBEdit
    Left = 443
    Top = 328
    Width = 82
    Height = 21
    CharCase = ecUpperCase
    DataField = 'vlr_outras_despesas'
    DataSource = DMPrincipal.DSMovimento
    Enabled = False
    TabOrder = 9
    OnExit = dbeOutrasDespesasExit
  end
  object rbCategoria: TRadioGroup
    Left = 8
    Top = 357
    Width = 625
    Height = 49
    Caption = 'Categoria do Movimento '
    Columns = 5
    Enabled = False
    ItemIndex = 0
    Items.Strings = (
      'Mercado'
      'Subscri'#231#227'o'
      'Desdobramento'
      'Bonifica'#231#227'o'
      'Outros')
    TabOrder = 16
  end
end
