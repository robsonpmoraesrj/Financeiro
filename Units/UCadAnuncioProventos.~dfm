object frmCadAnuncioProventos: TfrmCadAnuncioProventos
  Left = 340
  Top = 191
  Width = 697
  Height = 461
  Caption = 'Cadastro de Proventos Anunciados - v.01'
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  OnActivate = FormActivate
  OnClose = FormClose
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object Label2: TLabel
    Left = 7
    Top = 259
    Width = 53
    Height = 13
    Caption = 'Dt An'#250'ncio'
  end
  object edtTipoInvest: TLabel
    Left = 105
    Top = 259
    Width = 24
    Height = 13
    Caption = 'Ativo'
  end
  object Label4: TLabel
    Left = 335
    Top = 259
    Width = 88
    Height = 13
    Caption = 'Tipo de Provvento'
  end
  object Label5: TLabel
    Left = 605
    Top = 259
    Width = 69
    Height = 13
    Caption = 'R$ Cota/A'#231#227'o'
  end
  object Label6: TLabel
    Left = 8
    Top = 300
    Width = 19
    Height = 13
    Caption = '%IR'
  end
  object Label7: TLabel
    Left = 86
    Top = 300
    Width = 28
    Height = 13
    Caption = 'R$ IR'
  end
  object Label8: TLabel
    Left = 435
    Top = 300
    Width = 68
    Height = 13
    Caption = 'Dt Pagamento'
  end
  object Label11: TLabel
    Left = 540
    Top = 300
    Width = 42
    Height = 13
    Caption = 'Situa'#231#227'o'
  end
  object Label3: TLabel
    Left = 201
    Top = 300
    Width = 53
    Height = 13
    Caption = 'R$ L'#237'quido'
  end
  object Label12: TLabel
    Left = 336
    Top = 300
    Width = 38
    Height = 13
    Caption = 'Dt COM'
  end
  object GroupBox1: TGroupBox
    Left = 8
    Top = 8
    Width = 673
    Height = 126
    Caption = 'AN'#218'NCIO DE PROVENTOS'
    TabOrder = 0
    object Label1: TLabel
      Left = 7
      Top = 84
      Width = 53
      Height = 13
      Caption = 'Dt An'#250'ncio'
    end
    object Label9: TLabel
      Left = 126
      Top = 84
      Width = 24
      Height = 13
      Caption = 'Ativo'
    end
    object Label10: TLabel
      Left = 432
      Top = 84
      Width = 42
      Height = 13
      Caption = 'Situa'#231#227'o'
    end
    object btnSair: TBitBtn
      Left = 588
      Top = 16
      Width = 75
      Height = 33
      Caption = 'Sai'
      TabOrder = 1
      OnClick = btnSairClick
      Glyph.Data = {
        
        F8F8FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      NumGlyphs = 2
    end
    object dbnProvento: TDBNavigator
      Left = 8
      Top = 24
      Width = 224
      Height = 25
      DataSource = DMPrincipal.DSAnuncioProvento
      VisibleButtons = [nbFirst, nbPrior, nbNext, nbLast]
      TabOrder = 0
      OnClick = dbnProventoClick
    end
    object mskeCompetenciaP: TMaskEdit
      Left = 8
      Top = 98
      Width = 72
      Height = 21
      Enabled = False
      EditMask = '!99/99/0000;1;_'
      MaxLength = 10
      TabOrder = 3
      Text = '  /  /    '
      OnExit = mskeCompetenciaPExit
    end
    object dbPesqStatus: TDBLookupComboBox
      Left = 432
      Top = 98
      Width = 233
      Height = 21
      DataField = 'cod_carteira'
      Enabled = False
      KeyField = 'cod_status'
      ListField = 'dsc_status'
      ListSource = DMPrincipal.DSPesqStatus
      TabOrder = 5
      OnClick = dbPesqStatusClick
    end
    object btnLimpar: TBitBtn
      Left = 589
      Top = 56
      Width = 74
      Height = 33
      Caption = 'Limpar'
      TabOrder = 2
      OnClick = btnLimparClick
      Glyph.Data = {
        
        FFFFFFFFFFFEFFFEFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    end
    object dbPesqAtivo: TDBLookupComboBox
      Left = 126
      Top = 98
      Width = 260
      Height = 21
      KeyField = 'cod_ativo'
      ListField = 'nom_ativo'
      ListSource = DMPrincipal.DSAtivo
      TabOrder = 4
      OnClick = dbPesqAtivoClick
    end
  end
  object dbgTipos: TDBGrid
    Left = 8
    Top = 138
    Width = 673
    Height = 111
    DataSource = DMPrincipal.DSAnuncioProvento
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
    TabOrder = 1
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'MS Sans Serif'
    TitleFont.Style = []
    OnCellClick = dbgTiposCellClick
    Columns = <
      item
        Expanded = False
        FieldName = 'dat_anuncio'
        Title.Caption = 'Dt An'#250'ncio'
        Width = 60
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'Ativo'
        Width = 53
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'dat_com'
        Title.Caption = 'Data Com'
        Width = 62
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'dat_pagamento'
        Title.Caption = 'Pagamento'
        Width = 62
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'TpProv'
        Title.Caption = 'Tipo'
        Width = 77
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'vlr_bruto'
        Title.Caption = '$ Cota/A'#231#227'o'
        Width = 68
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'pct_ir'
        Title.Caption = '% IR'
        Width = 41
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'vlr_ir'
        Title.Caption = '$ IR'
        Width = 56
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'vlr_liquido'
        Title.Caption = '$ L'#237'quido'
        Width = 66
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'Situacao'
        Width = 91
        Visible = True
      end>
  end
  object mskeCompetencia: TMaskEdit
    Left = 8
    Top = 276
    Width = 71
    Height = 21
    Enabled = False
    EditMask = '!99/99/0000;1;_'
    MaxLength = 10
    TabOrder = 2
    Text = '  /  /    '
  end
  object dblcbAtivo: TDBLookupComboBox
    Left = 104
    Top = 276
    Width = 201
    Height = 21
    DataField = 'cod_ativo'
    DataSource = DMPrincipal.DSAnuncioProvento
    Enabled = False
    KeyField = 'cod_ativo'
    ListField = 'nom_ativo'
    ListSource = DMPrincipal.DSAtivo
    TabOrder = 3
  end
  object dblcbTipoProvento: TDBLookupComboBox
    Left = 336
    Top = 276
    Width = 222
    Height = 21
    DataField = 'cod_tipprovento'
    DataSource = DMPrincipal.DSAnuncioProvento
    Enabled = False
    KeyField = 'cod_tipprovento'
    ListField = 'dsc_tipprovento'
    ListSource = DMPrincipal.DSTpProvento
    TabOrder = 4
    OnClick = dblcbTipoProventoClick
  end
  object dbeValorUnitario: TDBEdit
    Left = 605
    Top = 276
    Width = 76
    Height = 21
    CharCase = ecUpperCase
    DataField = 'vlr_bruto'
    DataSource = DMPrincipal.DSAnuncioProvento
    Enabled = False
    TabOrder = 5
    OnExit = dbeValorUnitarioExit
  end
  object dbeCotas: TDBEdit
    Left = 8
    Top = 315
    Width = 69
    Height = 21
    CharCase = ecUpperCase
    Color = clActiveBorder
    DataField = 'pct_ir'
    DataSource = DMPrincipal.DSAnuncioProvento
    Enabled = False
    TabOrder = 6
    OnExit = dbeCotasExit
  end
  object edtTotProv: TEdit
    Left = 87
    Top = 315
    Width = 106
    Height = 21
    Color = clActiveBorder
    Enabled = False
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlue
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
    TabOrder = 7
    Text = 'edtTotProv'
  end
  object mskePagto: TMaskEdit
    Left = 436
    Top = 315
    Width = 72
    Height = 21
    Enabled = False
    EditMask = '!99/99/0000;1;_'
    MaxLength = 10
    TabOrder = 10
    Text = '  /  /    '
  end
  object dblcbSituacao: TDBLookupComboBox
    Left = 539
    Top = 315
    Width = 142
    Height = 21
    DataField = 'cod_status'
    DataSource = DMPrincipal.DSAnuncioProvento
    Enabled = False
    KeyField = 'cod_status'
    ListField = 'dsc_status'
    ListSource = DMPrincipal.DSStatus
    TabOrder = 11
  end
  object btnNovo: TBitBtn
    Left = 287
    Top = 387
    Width = 75
    Height = 31
    Caption = 'Novo'
    TabOrder = 12
    OnClick = btnNovoClick
    Glyph.Data = {
      
      9E9EFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
  end
  object btnSalvar: TBitBtn
    Left = 366
    Top = 387
    Width = 75
    Height = 31
    Caption = 'Salva'
    Enabled = False
    TabOrder = 13
    OnClick = btnSalvarClick
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
  end
  object btnAlterar: TBitBtn
    Left = 446
    Top = 387
    Width = 75
    Height = 31
    Caption = 'Altera'
    TabOrder = 14
    OnClick = btnAlterarClick
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
  end
  object btnCancelar: TBitBtn
    Left = 526
    Top = 387
    Width = 75
    Height = 31
    Caption = 'Cancela'
    Enabled = False
    TabOrder = 15
    OnClick = btnCancelarClick
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
  end
  object btnExcluir: TBitBtn
    Left = 605
    Top = 387
    Width = 75
    Height = 31
    Caption = 'Exclui'
    TabOrder = 16
    OnClick = btnExcluirClick
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
  end
  object edtTotProvLiq: TEdit
    Left = 199
    Top = 315
    Width = 106
    Height = 21
    Enabled = False
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlue
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
    TabOrder = 8
    Text = 'edtTotProvLiq'
  end
  object mskeCOM: TMaskEdit
    Left = 335
    Top = 315
    Width = 72
    Height = 21
    Enabled = False
    EditMask = '!99/99/0000;1;_'
    MaxLength = 10
    TabOrder = 9
    Text = '  /  /    '
  end
end
