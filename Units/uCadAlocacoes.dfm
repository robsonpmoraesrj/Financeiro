object frmCadAlocacoes: TfrmCadAlocacoes
  Left = 442
  Top = 169
  Width = 652
  Height = 531
  Caption = 'Cadastro de Aloca'#231#245'es de Ativos'
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnActivate = FormActivate
  OnClose = FormClose
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object edtTipoInvest: TLabel
    Left = 8
    Top = 373
    Width = 24
    Height = 13
    Caption = 'Ativo'
  end
  object edtInvestidor: TLabel
    Left = 8
    Top = 413
    Width = 46
    Height = 13
    Caption = 'Investidor'
  end
  object edtStatus: TLabel
    Left = 506
    Top = 413
    Width = 30
    Height = 13
    Caption = 'Status'
  end
  object Label2: TLabel
    Left = 8
    Top = 293
    Width = 126
    Height = 13
    Caption = 'Descri'#231#227'o do Investimento'
  end
  object Label3: TLabel
    Left = 387
    Top = 333
    Width = 68
    Height = 13
    Caption = 'Data de In'#237'cio'
  end
  object Label4: TLabel
    Left = 466
    Top = 333
    Width = 60
    Height = 13
    Caption = 'Data Vencto'
  end
  object Label5: TLabel
    Left = 8
    Top = 333
    Width = 63
    Height = 13
    Caption = 'Observa'#231#245'es'
  end
  object Label6: TLabel
    Left = 419
    Top = 413
    Width = 27
    Height = 13
    Caption = 'Cotas'
  end
  object Label7: TLabel
    Left = 387
    Top = 373
    Width = 36
    Height = 13
    Caption = 'Carteira'
  end
  object Label8: TLabel
    Left = 466
    Top = 292
    Width = 66
    Height = 13
    Caption = 'Valor Alocado'
  end
  object Label9: TLabel
    Left = 551
    Top = 292
    Width = 76
    Height = 13
    Caption = 'Valor Atualizado'
  end
  object Label10: TLabel
    Left = 550
    Top = 333
    Width = 81
    Height = 13
    Caption = 'Data Atualiza'#231#227'o'
  end
  object Label11: TLabel
    Left = 196
    Top = 373
    Width = 21
    Height = 13
    Caption = 'Tipo'
  end
  object GroupBox1: TGroupBox
    Left = 8
    Top = 8
    Width = 625
    Height = 151
    Caption = 'ALOCA'#199#213'ES'
    TabOrder = 0
    object Label1: TLabel
      Left = 8
      Top = 56
      Width = 126
      Height = 13
      Caption = 'Descri'#231#227'o do Investimento'
    end
    object Label12: TLabel
      Left = 8
      Top = 98
      Width = 36
      Height = 13
      Caption = 'Carteira'
    end
    object Label13: TLabel
      Left = 317
      Top = 55
      Width = 24
      Height = 13
      Caption = 'Ativo'
    end
    object Label14: TLabel
      Left = 316
      Top = 98
      Width = 99
      Height = 13
      Caption = 'Tipo de Investimento'
    end
    object btnSair: TBitBtn
      Left = 538
      Top = 21
      Width = 75
      Height = 33
      Caption = 'Sai'
      TabOrder = 5
      OnClick = btnSairClick
      Glyph.Data = {
        
        F8F8FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      NumGlyphs = 2
    end
    object edtProcurar: TEdit
      Left = 8
      Top = 72
      Width = 305
      Height = 21
      CharCase = ecUpperCase
      Enabled = False
      TabOrder = 1
      OnKeyUp = edtProcurarKeyUp
    end
    object dbnAlocacao: TDBNavigator
      Left = 8
      Top = 24
      Width = 224
      Height = 25
      DataSource = DMPrincipal.DSAlocacao
      VisibleButtons = [nbFirst, nbPrior, nbNext, nbLast]
      TabOrder = 0
      OnClick = dbnAlocacaoClick
    end
    object btnLimpar: TBitBtn
      Left = 539
      Top = 62
      Width = 74
      Height = 33
      Caption = 'Limpar'
      TabOrder = 6
      OnClick = btnLimparClick
      Glyph.Data = {
        
        FFFFFFFFFFFEFFFEFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    end
    object dbPesqCarteira: TDBLookupComboBox
      Left = 8
      Top = 114
      Width = 305
      Height = 21
      KeyField = 'cod_carteira'
      ListField = 'nom_carteira'
      ListSource = DMPrincipal.DSCarteira
      TabOrder = 3
      OnClick = dbPesqCarteiraClick
    end
    object dblkAtivo: TDBLookupComboBox
      Left = 317
      Top = 71
      Width = 212
      Height = 21
      KeyField = 'cod_ativo'
      ListField = 'nom_ativo'
      ListSource = DMPrincipal.DSAtivo
      TabOrder = 2
      OnClick = dblkAtivoClick
    end
    object dblkTipoInvest: TDBLookupComboBox
      Left = 316
      Top = 114
      Width = 298
      Height = 21
      KeyField = 'cod_tipoinvestimento'
      ListField = 'dsc_tipoinvestimento'
      ListSource = DMPrincipal.DSTipInvest
      TabOrder = 4
      OnClick = dblkTipoInvestClick
    end
  end
  object dbgAlocacoes: TDBGrid
    Left = 8
    Top = 165
    Width = 625
    Height = 123
    DataSource = DMPrincipal.DSAlocacao
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
    ReadOnly = True
    TabOrder = 1
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'MS Sans Serif'
    TitleFont.Style = []
    OnCellClick = dbgAlocacoesCellClick
    Columns = <
      item
        Expanded = False
        FieldName = 'dsc_investimento'
        Title.Caption = 'Ativo'
        Width = 135
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'Carteira'
        Width = 82
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'Tipo'
        Width = 85
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'nro_cotas'
        Title.Caption = 'Cotas'
        Width = 58
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'vlr_alocado'
        Title.Caption = 'Vlr. Alocado'
        Width = 77
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'vlr_atualizado'
        Title.Caption = 'Vlr. Atualizado'
        Width = 75
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'dat_atualizacao'
        Title.Caption = 'Data Atualiza'#231#227'o'
        Width = 86
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'Investidor'
        Width = 93
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'Situacao'
        Title.Caption = 'Situa'#231#227'o'
        Width = 69
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'dat_inicio_investimento'
        Title.Caption = 'In'#237'cio'
        Width = 54
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'dat_fim_investimento'
        Title.Caption = 'Fim'
        Width = 55
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'dsc_observacao'
        Title.Caption = 'Observa'#231#227'o'
        Width = 152
        Visible = True
      end>
  end
  object btnNovo: TBitBtn
    Left = 239
    Top = 459
    Width = 75
    Height = 31
    Caption = 'Novo'
    TabOrder = 15
    OnClick = btnNovoClick
    Glyph.Data = {
      
      9E9EFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
  end
  object btnSalvar: TBitBtn
    Left = 318
    Top = 459
    Width = 75
    Height = 31
    Caption = 'Salva'
    Enabled = False
    TabOrder = 16
    OnClick = btnSalvarClick
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
  end
  object btnAlterar: TBitBtn
    Left = 398
    Top = 459
    Width = 75
    Height = 31
    Caption = 'Altera'
    TabOrder = 17
    OnClick = btnAlterarClick
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
  end
  object btnCancelar: TBitBtn
    Left = 478
    Top = 459
    Width = 75
    Height = 31
    Caption = 'Cancela'
    Enabled = False
    TabOrder = 18
    OnClick = btnCancelarClick
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
  end
  object btnExcluir: TBitBtn
    Left = 557
    Top = 459
    Width = 75
    Height = 31
    Caption = 'Exclui'
    TabOrder = 19
    OnClick = btnExcluirClick
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
  end
  object dblcbAtivo: TDBLookupComboBox
    Left = 8
    Top = 389
    Width = 185
    Height = 21
    DataField = 'cod_ativo'
    DataSource = DMPrincipal.DSAlocacao
    KeyField = 'cod_ativo'
    ListField = 'nom_ativo'
    ListSource = DMPrincipal.DSAtivo
    TabOrder = 9
  end
  object dblcbInvestidor: TDBLookupComboBox
    Left = 8
    Top = 429
    Width = 401
    Height = 21
    DataField = 'cod_proprietario'
    DataSource = DMPrincipal.DSAlocacao
    KeyField = 'cod_proprietario'
    ListField = 'nom_proprietario'
    ListSource = DMPrincipal.DSInvestidor
    TabOrder = 12
  end
  object dblcbStatus: TDBLookupComboBox
    Left = 505
    Top = 428
    Width = 129
    Height = 21
    DataField = 'cod_status'
    DataSource = DMPrincipal.DSAlocacao
    KeyField = 'cod_status'
    ListField = 'dsc_status'
    ListSource = DMPrincipal.DSStatus
    TabOrder = 14
  end
  object dbeAlocacao: TDBEdit
    Left = 8
    Top = 309
    Width = 449
    Height = 21
    CharCase = ecUpperCase
    DataField = 'dsc_investimento'
    DataSource = DMPrincipal.DSAlocacao
    Enabled = False
    TabOrder = 2
  end
  object dbeObs: TDBEdit
    Left = 8
    Top = 349
    Width = 369
    Height = 21
    CharCase = ecUpperCase
    DataField = 'dsc_observacao'
    DataSource = DMPrincipal.DSAlocacao
    Enabled = False
    TabOrder = 5
  end
  object dbeCotas: TDBEdit
    Left = 417
    Top = 429
    Width = 85
    Height = 21
    CharCase = ecUpperCase
    DataField = 'nro_cotas'
    DataSource = DMPrincipal.DSAlocacao
    Enabled = False
    TabOrder = 13
  end
  object dblcbCarteira: TDBLookupComboBox
    Left = 386
    Top = 389
    Width = 247
    Height = 21
    DataField = 'cod_carteira'
    DataSource = DMPrincipal.DSAlocacao
    KeyField = 'cod_carteira'
    ListField = 'nom_carteira'
    ListSource = DMPrincipal.DSCarteira
    TabOrder = 11
  end
  object dbeValorAlocado: TDBEdit
    Left = 466
    Top = 308
    Width = 81
    Height = 21
    CharCase = ecUpperCase
    DataField = 'vlr_alocado'
    DataSource = DMPrincipal.DSAlocacao
    Enabled = False
    TabOrder = 3
  end
  object dbeValorAtualizado: TDBEdit
    Left = 551
    Top = 308
    Width = 83
    Height = 21
    CharCase = ecUpperCase
    DataField = 'vlr_atualizado'
    DataSource = DMPrincipal.DSAlocacao
    Enabled = False
    TabOrder = 4
  end
  object dblxTipo: TDBLookupComboBox
    Left = 195
    Top = 389
    Width = 182
    Height = 21
    DataField = 'cod_tipoinvestimento'
    DataSource = DMPrincipal.DSAlocacao
    KeyField = 'cod_tipoinvestimento'
    ListField = 'dsc_tipoinvestimento'
    ListSource = DMPrincipal.DSTipInvest
    TabOrder = 10
  end
  object DTPDtAtualizacao: TDateTimePicker
    Left = 549
    Top = 348
    Width = 85
    Height = 21
    Date = 44592.382542141200000000
    Time = 44592.382542141200000000
    TabOrder = 8
  end
  object dbeInicio: TMaskEdit
    Left = 387
    Top = 349
    Width = 72
    Height = 21
    Enabled = False
    EditMask = '!99/99/0000;1;_'
    MaxLength = 10
    TabOrder = 6
    Text = '  /  /    '
  end
  object dbeVencto: TMaskEdit
    Left = 466
    Top = 349
    Width = 72
    Height = 21
    Enabled = False
    EditMask = '!99/99/0000;1;_'
    MaxLength = 10
    TabOrder = 7
    Text = '  /  /    '
  end
end
